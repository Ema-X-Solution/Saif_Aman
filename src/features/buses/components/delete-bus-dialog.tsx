...
export function DeleteBusDialog({ bus, onClose, onDeleted }: DeleteBusDialogProps) {
  const t = useT();
  ...
  async function handleDelete() {
    ...
    setIsDeleting(true);
    try {
      await busesService.remove(bus.id);
  ...
  return (
    <Dialog open={!!bus} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-md" dir={dialogDir}>
        ...
        <DialogDescription>
          {t("common.confirmDelete")} <span className="font-semibold">{bus?.plateNumber}</span>?
        </DialogDescription>
